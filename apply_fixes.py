...
for i, line in enumerate(lines):
    if "final jsonMatch = RegExp(r'\\[[\\s\\S]*\\]').firstMatch(cleanedContent);" in line:
        lines[i] = "        final jsonMatch = RegExp(r'(\\[[\\s\\S]*\\]|\\{[\\s\\S]*\\})').firstMatch(cleanedContent);\n"
...
if process_start != -1:
    ...
    new_process_chunk = """    if (itemsToProcess.isNotEmpty) {
      ...
      for (int i = 0; i < itemsToProcess.length; i += chunkSize) {
        ...
        final endIndex = i + chunkSize < itemsToProcess.length ? i + chunkSize : itemsToProcess.length;
        setState(() {
...
score_start = -1
for i, line in enumerate(lines):
    if "    if (wasWinnerThisGame) {" in line and "final pointsGained = playerScores.length - 1;" in lines[i+1]:
        score_start = i
        break
...
if score_start != -1:
    ...
    for i in range(score_start, len(lines)):
        if "final theme = await _extractThemeFromText" in lines[i]:
            score_end = i
            break
            
    new_score_chunk = """    final pointsGained = playerScores.length - 1;
    bool isReplay = false;
    if (quizId != null) {
    ...
    try {
      ...
      print("Erreur lors de la transaction pour $playerName: $e");
    }

    if (wasWinnerThisGame && pointsGained > 0 && context.mounted) {
